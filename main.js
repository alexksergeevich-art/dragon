const VER = "dragon-polish-20260307-lvl3-eqsize-glow";
...
const HERO_CONFIG = {
  ...
  3: { id:"lvl3", ringIndex:2, maxHp:270, damage:44, speed:176, attackRange:94, attackCooldown:0.74, dragonThreat:38 },
};
...
function heroGlowFilter(level){
  if (level === 3){
    return [
      "drop-shadow(0 0 8px rgba(255,255,255,.30))",
      "drop-shadow(0 0 14px rgba(185,245,255,.42))",
      "drop-shadow(0 0 22px rgba(90,210,255,.34))",
      "drop-shadow(0 0 32px rgba(55,120,255,.22))"
    ].join(" ");
  }
  if (level >= 4){
    return [
      "drop-shadow(0 0 8px rgba(255,255,255,.24))",
      "drop-shadow(0 0 18px rgba(255,210,120,.26))"
    ].join(" ");
  }
  return "";
}
...
function updateHero(hero, dt){
  ...
  const glow = heroGlowFilter(hero.level);
  const takingHit = hero.alive && hero.hp < hero.maxHp;
  hero.spriteEl.style.filter = glow || takingHit
    ? `${glow}${glow && takingHit ? " " : ""}${takingHit ? "brightness(1.08) saturate(1.05)" : ""}`.trim()
    : "";
...
function bindUi(){
  ...
  stage.addEventListener("pointerdown", (ev) => {
    ...
    if (!hero){
      status("Сначала заспавни героя кнопкой 1, 2 или 3. Кнопки 4 и 5 пока заглушки.");
      return;
    }
    if (!hero.alive){
...
bindUi();
resize();
status(`Анализ фона: ${BG_ANALYSIS.notes.spawnBand}, глубина сохранена, LVL 1–3 выровнены по размеру, LVL 3 получил сине-голубое свечение.`);
