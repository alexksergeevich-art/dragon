const VER = "dragon-polish-20260307-lvl3-equalized";
...
const HERO_BASE_SCENE_HEIGHT = 145;
...
const SPRITES = {
  ...
  lvl3: {
    atlasUrl:  `assets/LVL3/lvl3_master_sprite_atlas_scaled.png?v=${VER}`,
    atlasCols: 8,
    atlasRows: 10,
    frameW: 464,
    frameH: 554,
    ...
    idleFront: { url:`assets/LVL3/lvl3_master_sprite_atlas_scaled.png?v=${VER}`, frameW:464, frameH:554, frames:8, fps:6,  row:5, atlas:true },
    idleBack:  { url:`assets/LVL3/lvl3_master_sprite_atlas_scaled.png?v=${VER}`, frameW:464, frameH:554, frames:8, fps:6,  row:6, atlas:true },
    attack:    { url:`assets/LVL3/lvl3_master_sprite_atlas_scaled.png?v=${VER}`, frameW:464, frameH:554, frames:8, fps:14, row:7, atlas:true, renderScale:1.0 },
...
function validateSpriteManifest(){
  const checks = [];
  const groups = [SPRITES.lvl1, SPRITES.lvl2, SPRITES.lvl3, SPRITES.dragon];
...
function getHeroBaseScale(spriteCfg){
  const refSprite = spriteCfg.idleBack || spriteCfg.idleFront || spriteCfg.down || spriteCfg.up || spriteCfg.right;
  const refFrameH = refSprite?.frameH || 1;
  return (HERO_BASE_SCENE_HEIGHT / refFrameH);
}
...
function normY(yPx){
  return clamp(yPx / Math.max(state.height, 1), 0, 1);
}
...
function createHero(level){
  ...
  dom.heroEl.classList.add(`hero-level-${level}`);
  if (spriteCfg.glowClass) dom.heroEl.classList.add(spriteCfg.glowClass);

  const hero = {
    level,
    cfg: heroCfg,
    sprites: spriteCfg,
    baseScale: getHeroBaseScale(spriteCfg),
    ...dom,
    x: spawnPos.x,
    y: spawnPos.y,
    spawnX: spawnPos.x,
...
function updateHero(hero, dt){
  ...
  const scale = depthScale(hero.y) * hero.baseScale * hero.sprites.drawScale;
  ...
  const spriteHeight = spriteMeta.frameH * scale * (spriteMeta.renderScale || 1);
  hero.hpBarEl.style.top = `${-spriteHeight - 14}px`;
  hero.labelEl.style.top = `${-spriteHeight - 30}px`;
  hero.selectionCircleEl.style.bottom = `${Math.max(-8, -spriteHeight * 0.04)}px`;
  if (hero.level >= 3){
    hero.spriteEl.style.filter = 'drop-shadow(0 0 6px rgba(120,210,255,.55)) drop-shadow(0 0 14px rgba(80,180,255,.35)) drop-shadow(0 0 24px rgba(255,255,255,.22))';
  } else {
    hero.spriteEl.style.filter = '';
  }
...
function bindUi(){
  ...
  stage.addEventListener("pointerdown", (ev) => {
    ...
    if (!hero){
      status("Сначала заспавни героя кнопкой 1, 2 или 3.");
      return;
    }
    if (!hero.alive){
...
bindUi();
resize();
status(`Анализ фона: ${BG_ANALYSIS.notes.spawnBand}, центр дороги и верхняя лужайка выделены. LVL 3 атлас подключен.`);
