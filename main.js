const VER = "sprite-align-20260306b";
...
const BG_ANALYSIS = {
  ...
  notes: {
    ...
    dragonField: "зелёная лужайка в верхней центральной части кадра"
  },
  rings: [
    { level: 1, x: 0.116, y: 0.788, w: 0.102, h: 0.032, power: 1.0 },
    { level: 2, x: 0.305, y: 0.790, w: 0.114, h: 0.034, power: 1.32 },
    { level: 3, x: 0.501, y: 0.789, w: 0.114, h: 0.034, power: 1.56 },
    { level: 4, x: 0.701, y: 0.787, w: 0.110, h: 0.033, power: 1.84 },
    { level: 5, x: 0.892, y: 0.784, w: 0.106, h: 0.032, power: 2.1 },
  ],
  dragon: { x: 0.515, y: 0.430 },
  walkable: { minY: 0.325, maxY: 0.87, nearMarginX: 0.06, farHalfWidth: 0.11, nearHalfWidth: 0.36 }
};

const SPRITES = {
  lvl1: {
    down:      { url:`assets/LVL1/sprite_down_lvl1.png?v=${VER}`,       frameW:354, frameH:346, frames:6,  fps:10, renderScale:1.00, uiLift:1.00 },
    right:     { url:`assets/LVL1/sprite_right_lvl1.png?v=${VER}`,      frameW:398, frameH:356, frames:6,  fps:10, renderScale:1.00, uiLift:1.00 },
    downRight: { url:`assets/LVL1/sprite_downright_lvl1.png?v=${VER}`,  frameW:493, frameH:358, frames:6,  fps:10, renderScale:1.00, uiLift:1.00 },
    upRight:   { url:`assets/LVL1/sprite_upright_lvl1.png?v=${VER}`,    frameW:306, frameH:306, frames:6,  fps:10, renderScale:1.00, uiLift:1.00 },
    up:        { url:`assets/LVL1/sprite_up_lvl1.png?v=${VER}`,         frameW:284, frameH:326, frames:8,  fps:10, renderScale:1.00, uiLift:1.00 },
    idleFront: { url:`assets/LVL1/sprite_idlefront_lvl1.png?v=${VER}`,  frameW:287, frameH:344, frames:6,  fps:6,  renderScale:1.00, uiLift:1.00 },
    idleBack:  { url:`assets/LVL1/sprite_idleback_lvl1.png?v=${VER}`,   frameW:237, frameH:338, frames:6,  fps:6,  renderScale:1.00, uiLift:1.00 },
    attack:    { url:`assets/LVL1/sprite_attack_lvl1.png?v=${VER}`,     frameW:251, frameH:386, frames:8,  fps:16, renderScale:1.42, uiLift:1.03 },
    death:     { url:`assets/LVL1/sprite_death_lvl1.png?v=${VER}`,      frameW:344, frameH:400, frames:10, fps:12, renderScale:1.00, uiLift:0.55 },
    drawScale: 0.37,
    uiName: "LVL 1"
  },
  lvl2: {
    down:      { url:`assets/LVL2/sprite_down.png?v=${VER}`,            frameW:688, frameH:464, frames:10, fps:12, renderScale:0.86, uiLift:1.00 },
    right:     { url:`assets/LVL2/sprite_righ.png?v=${VER}`,            frameW:292, frameH:293, frames:30, fps:12, renderScale:1.00, uiLift:1.00 },
    downRight: { url:`assets/LVL2/sprite_down_right.png?v=${VER}`,      frameW:688, frameH:464, frames:6,  fps:12, renderScale:0.86, uiLift:1.00 },
    upRight:   { url:`assets/LVL2/sprite_up_right.png?v=${VER}`,        frameW:688, frameH:464, frames:24, fps:14, renderScale:0.86, uiLift:1.00 },
    up:        { url:`assets/LVL2/sprite_up.png?v=${VER}`,              frameW:332, frameH:302, frames:12, fps:12, renderScale:1.00, uiLift:1.00 },
    idleFront: { url:`assets/LVL2/sprite_idle_front.png?v=${VER}`,      frameW:688, frameH:464, frames:7,  fps:6,  renderScale:0.86, uiLift:1.00 },
    idleBack:  { url:`assets/LVL2/sprite_idle_back.png?v=${VER}`,       frameW:353, frameH:342, frames:12, fps:6,  renderScale:1.00, uiLift:1.00 },
    attack:    { url:`assets/LVL2/sprite_attack.png?v=${VER}`,          frameW:459, frameH:392, frames:24, fps:16, renderScale:0.96, uiLift:1.00 },
    death:     { url:`assets/LVL2/sprite_death.png?v=${VER}`,           frameW:688, frameH:464, frames:23, fps:12, renderScale:0.84, uiLift:0.55 },
    drawScale: 0.34,
    uiName: "LVL 2"
  },
  dragon: {
    idle:      { url:`assets/dragon/sprite_dragon.png?v=${VER}`,        frameW:256, frameH:256, frames:24, fps:12, renderScale:1.00, uiLift:1.00 },
    drawScale: 0.72,
    uiName: "Dragon"
  }
};
...
function depthT(yPx){
  ...
  return clamp((yPx - minY) / Math.max(maxY - minY, 1), 0, 1);
}
function depthScale(yPx){
  const t = depthT(yPx);
  return lerp(0.27, 0.78, Math.pow(t, 1.18));
}
function depthSpeed(yPx){
  const t = depthT(yPx);
  return lerp(0.54, 1.0, Math.pow(t, 1.08));
}
function corridorHalfWidth(yPx){
  const t = depthT(yPx);
  return lerp(BG_ANALYSIS.walkable.farHalfWidth, BG_ANALYSIS.walkable.nearHalfWidth, Math.pow(t, 1.08)) * state.width;
}
...
function clampToWalkable(pos){
  ...
  const halfWidth = corridorHalfWidth(y);
  const nearMarginX = BG_ANALYSIS.walkable.nearMarginX * state.width;
  return {
    x: clamp(pos.x, Math.max(nearMarginX, centerX - halfWidth), Math.min(state.width - nearMarginX, centerX + halfWidth)),
    y,
  };
}
...
function makeHeroDom(level){
  ...
  const teleportGlow = document.createElement("div");
  teleportGlow.className = "teleportGlow";
  const teleportBeam = document.createElement("div");
  teleportBeam.className = "teleportBeam";

  heroEl.append(label, hpbar, selectionCircle, teleportGlow, teleportBeam, sprite);
...
function createHero(level){
  ...
  const spawnPos = worldToStage(ring.x, ring.y + 0.001);
...
function applySpriteToEl(spriteEl, spriteMeta, scale, frame, flipX){
  const renderScale = spriteMeta.renderScale || 1;
  const finalScale = scale * renderScale;
  spriteEl.style.width = `${spriteMeta.frameW * finalScale}px`;
  spriteEl.style.height = `${spriteMeta.frameH * finalScale}px`;
  spriteEl.style.backgroundImage = `url("${spriteMeta.url}")`;
  spriteEl.style.backgroundSize = `${spriteMeta.frameW * spriteMeta.frames * finalScale}px ${spriteMeta.frameH * finalScale}px`;
  spriteEl.style.backgroundPosition = `${-frame * spriteMeta.frameW * finalScale}px 0px`;
  spriteEl.style.transform = `translateX(-50%) scaleX(${flipX ? -1 : 1})`;
  return {
    width: spriteMeta.frameW * finalScale,
    height: spriteMeta.frameH * finalScale,
    finalScale,
  };
}
...
function updateHero(hero, dt){
  ...
  const spriteBox = applySpriteToEl(hero.spriteEl, spriteMeta, scale, hero.frame, hero.flipX);
  const uiLift = spriteMeta.uiLift || 1;
  const uiTop = -spriteBox.height * uiLift;
  hero.labelEl.style.top = `${uiTop - 24}px`;
  hero.labelEl.style.fontSize = `${clamp(spriteBox.width * 0.075, 14, 20)}px`;
  hero.labelEl.style.maxWidth = `${Math.max(120, spriteBox.width * 1.2)}px`;
  hero.hpFillEl.parentElement.style.top = `${uiTop - 2}px`;
  hero.hpFillEl.parentElement.style.width = `${clamp(spriteBox.width * 0.72, 110, 168)}px`;
...
function updateDragon(dt){
  ...
  const dragonBox = applySpriteToEl(dragonSpriteEl, dragonMeta, scale, state.dragon.frame, false);
  dragonLabel.style.top = `${-dragonBox.height - 26}px`;
  dragonLabel.style.fontSize = `${clamp(dragonBox.width * 0.11, 16, 22)}px`;
  dragonLabel.style.maxWidth = `${Math.max(150, dragonBox.width * 1.3)}px`;
  dragonHPFill.parentElement.style.top = `${-dragonBox.height - 4}px`;
  dragonHPFill.parentElement.style.width = `${clamp(dragonBox.width * 0.92, 140, 200)}px`;
}
...
function resize(){
  ...
  for (const hero of state.heroes.values()){
    const ring = BG_ANALYSIS.rings[hero.level - 1];
    const targetRingPos = worldToStage(ring.x, ring.y + 0.001);
