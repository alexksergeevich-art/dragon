...
function heroDepthScale(yPx){
  ...
  // Mobile uses a slightly more aggressive curve so it visually matches desktop.
  const curve = mobile
    ? [
        [0.00, 0.15],
        [0.20, 0.17],
...
function snapStableScale(entity, nextScale, holdStable = false){
  ...
  if (holdStable && prev !== undefined && Math.abs(scale - prev) < 0.0035){
    scale = prev;
  }
