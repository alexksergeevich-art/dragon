const VER = "dragon-polish-20260307-diablo-depthfix";
...
function depthNorm(yPx){
  return clamp(yPx / Math.max(state.height, 1), 0, 1);
}
function depthScale(yPx){
  const t = depthNorm(yPx);

  // Diablo-like perspective:
  // top (dragon zone) = much smaller, bridge end = mid size, bottom = full size
  return sampleTrack(t, [
    [0.00, 0.50],
    [0.18, 0.54],
    [0.30, 0.60],
    [0.40, 0.66],
...
function depthSpeed(yPx){
  ...
  // slower near dragon, normal near camera
  return sampleTrack(t, [
    [0.00, 0.42],
    [0.18, 0.50],
    [0.40, 0.62],
...
function diabloPerspectiveShift(yPx){
  const t = depthNorm(yPx);
  return lerp(-state.width * 0.012, state.width * 0.010, t);
}
function snapStableScale(entity, nextScale){
  const prev = entity._stableScale;
  let scale = nextScale;
  if (prev !== undefined && Math.abs(scale - prev) < 0.0035){
    scale = prev;
  }
...
function combatRange(baseRange){
  return baseRange / 2.5;
}

const HERO_VISIBLE_BASE = {
  lvl1: 312,
  lvl2: 315,
  lvl3: 312,
};

function isDesktopSizeBoost(){
  return window.matchMedia('(min-width: 921px)').matches;
}

function heroVisibleTargetPx(){
  return isDesktopSizeBoost() ? 335 : 200;
}
...
function updateHero(hero, dt){
  ...
  const rawScale = depthScale(hero.y) * hero.baseScale;
  const scale = snapStableScale(hero, rawScale);
...
function updateDragon(dt){
  ...
  const rawDragonScale = depthScale(state.dragon.y) * SPRITES.dragon.drawScale * 0.96;
...
function resize(){
  ...
  state.dragon.y = dragonPos.y - state.height * 0.032;
...
bindUi();
resize();
status(`LVL 1 / 2 / 3 выровнены по визуальному размеру. На мобильном спавн x1.5, на ПК x2.5.`);
